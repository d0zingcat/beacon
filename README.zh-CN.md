...
Beacon 内置 **24 个数据源**。部署后请执行 D1 迁移，以注册 RSS 订阅类源。
...
| `bedrock-docs` | append | feed | [AWS Bedrock User Guide 更新](https://docs.aws.amazon.com/bedrock/latest/userguide/) RSS |
...
3. 部署完成后，在 Cloudflare Dashboard → Worker → **Settings → Variables and Secrets** 中配置：
   ...
   - `WEBHOOK_ENCRYPTION_KEY` — 32 字节密钥，用于加密用户自己的飞书 Webhook
...
pnpm exec wrangler secret put WEBHOOK_ENCRYPTION_KEY # 用户飞书订阅需要
...
| `WEBHOOK_ENCRYPTION_KEY` | 32 字节密钥，用于加密用户自己的飞书 Webhook URL | 用户订阅需要 |
| `APP_ENV` | 设为 `local` 时，本地开发会把 magic link 打到日志，而不是要求 Email binding | 否 |

至少配置一种全局通知渠道后，部署级通知才会推送。用户自行管理的飞书订阅是独立能力，需要配置 `WEBHOOK_ENCRYPTION_KEY`。生产环境的邮箱 magic link 登录使用 Cloudflare Email Sending 的 `EMAIL` binding；本地开发可设置 `APP_ENV=local`，让登录链接输出到日志。D1 / Queue / Browser 通过 wrangler bindings 注入，无需单独配置。

### Web UI 与订阅

Beacon 由同一个 Worker 提供轻量网页界面：

| 路径 | 说明 |
|------|------|
| `/` | 公开的近期条目流 |
| `/browse/sources` | 公开数据源目录 |
| `/browse/sources/:id` | 公开数据源详情与近期条目 |
| `/login` | 邮箱 magic-link 登录 |
| `/app/subscriptions` | 登录后的飞书订阅中心 |
